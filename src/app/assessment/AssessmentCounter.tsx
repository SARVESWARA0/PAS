...
import { ChevronRight, Clock, Check, AlertCircle, ChevronLeft } from "lucide-react"
...
const QuestionNavigationBar = ({ totalQuestions, currentQuestion, responses, currentSection, onQuestionSelect }) => {
  return (
    <div className="fixed left-0 top-0 h-full w-20 bg-gray-900 flex flex-col items-center py-6 border-r border-indigo-500/30 overflow-y-auto">
      {Array.from({ length: totalQuestions }).map((_, index) => {
        const questionNumber = index + 1
        const isCompleted = responses[currentSection]?.[index]?.response?.trim().length > 0
        ...
        return (
          <button
            ...
            className={`
              ...
              font-medium text-sm border mb-3 transition-all duration-200
              ${
                isCompleted
              ...
              ${isCurrent ? "ring-2 ring-indigo-500 ring-offset-2 ring-offset-gray-900" : ""}
              hover:bg-indigo-700 hover:border-indigo-500 hover:text-white
            `}
          >
            {isCompleted ? <Check className="w-5 h-5" /> : questionNumber}

            {isCurrent && (
              <span className="absolute -right-24 top-1/2 transform -translate-y-1/2 text-indigo-300 text-xs whitespace-nowrap bg-gray-800 px-2 py-1 rounded">
                Current
              </span>
            )}
          </button>
...
export default function AssessmentContent({ userName }: AssessmentContentProps) {
  ...
  const [timing, setTiming] = useState<{
    [key: number]: { startTime?: number; timeLeft?: number; timeTaken?: number }
  }>({})
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900 flex">
      ...
      />
      <div className="flex-1 pl-24 min-h-screen">
        <div className="max-w-4xl mx-auto px-6 py-12">
          <div className="bg-gray-800/80 backdrop-blur-lg shadow-2xl rounded-3xl overflow-hidden border border-indigo-500/30">
            <div className="px-8 py-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white relative overflow-hidden">
              ...
              <h1 className="text-4xl font-bold tracking-tight relative z-10">Welcome, {userName}</h1>
            ...
            <div className="p-8">
              ...
              <textarea
                className="w-full p-6 bg-gray-700/80 text-gray-100 border border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 ease-in-out hover:bg-gray-600/80 text-lg resize-none"
                rows={8}
              ...
              <div className="mt-8 space-y-6">
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    <label htmlFor="blockCopyPaste" className="mr-3 text-indigo-300">
                      Block Copy/Paste
                    </label>
                    <input
                      type="checkbox"
